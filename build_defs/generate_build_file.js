...
load("//build_defs:typescript.bzl", "ts_declaration")
...
`;
  ...
  for (const def of targetDefs) {
    let fn;
    const args = {
      name: `"${escapeTarget(def.name)}"`,
      // srcs: `[${def.srcs.map(src => `"${src}"`).join(', ')}]`,
    };
    if (!def.typings) {
      fn = 'nodejs_module';
      args.deps = `[${def.deps.map(dep => `":${escapeTarget(dep)}"`).join(', ')}]`;
      // skip for now
      continue;
    } else {
      fn = 'ts_declaration';
      ...
      args.srcs = `glob(["${path.join(escapeFile(def.name), '**/*.d.ts')}"])`;
      // args.ambient = def.typings.ambient ? 'True' : 'False';
      // args.entry_point = `"${path.join(def.name, def.typings.path)}"`;
      // args.root_dir = `"${path.join(def.name)}"`;
    }
    output += `${fn}(${Object.keys(args).map(k => `${k}=${args[k]}`).join(', ')})\n`;
  }

  fs.writeFileSync(path.join(nodeModules, 'BUILD'), output, {encoding: 'utf-8'});
}
...
function tracePackage(packageDir) {
  ...
  if (packageJson) {
    ...
    if (typeof packageJson.typings === 'string')
      typingsPath = packageJson.typings;
    else if (typeof packageJson.types === 'string')
      typingsPath = packageJson.types;
