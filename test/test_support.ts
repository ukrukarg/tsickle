...
export function compile(
  ...
  if (!compiledJS) {
    assert.fail(`Couldn't find file ${compiledJSFileName} in compilation output files: ${JSON.stringify(toArray(jsFiles.keys()))}`);
    throw new Error('unreachable');
  }
  ...
  const dts = getFileWithName(compiledJSFileName.substring(0, compiledJSFileName.length - 3) + '.d.ts', jsFiles);
